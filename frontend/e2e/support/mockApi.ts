...
async function handle(request: Request, path: string, state: MockState): Promise<{ body?: unknown; status?: number }> {
  ...
  if (path === "/system/capabilities" && method === "GET") return { body: capabilitiesFixture() };
...
const capabilitiesFixture = () => ({ flags: { ai_configured: false, supervised_browser_available: false, portfolio_demo: true }, states: { industry_trend_analysis: { state: "available" }, career_agent_recommendations: { state: "available" }, archetype_performance: { state: "available" } }, integrations: [], labels: {} });
