...
import {
  ...
  executeSubmissionQueueItem, generateBreakdownStrategy, getDiscoveryReport, listAuditionReadiness,
...
export const breakdownKeys = {
  ...
  discoveryReport: queryKeys.breakdowns.list({ resource: "discoveryReport" })
} as const;
...
export const useDiscoveryReport = () => useQuery({ queryKey: breakdownKeys.discoveryReport, queryFn: getDiscoveryReport, ...workflow });
...
export function useRunBreakdownDiscovery() { const invalidate = useInvalidator(); return useMutation({ mutationFn: (request: DiscoveryRunRequest) => runBreakdownDiscovery(request), onSuccess: () => invalidate(...classificationKeys, ...opportunityAggregateKeys, breakdownKeys.recommendations, breakdownKeys.discoveryReport, sourceListKey, sourceArchiveKey) }); }
