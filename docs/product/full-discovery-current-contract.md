...
10. Registry `run_source` and the configured public-web orchestration boundary each own their transaction. One configured public-web execution creates one `DiscoveryRun`; accepted-visible and review-hidden Opportunities, travel exceptions, and intended source suggestions commit with the successful run. Newly created rejected-discarded Opportunities are deleted before commit. A systemic failure rolls back all partial domain writes, then best-effort persists exactly one failed run from a clean transaction without masking the original exception. Disabled/skipped public-web behavior does not create a run.
...
|---|---|---|---|---|---|---|---|---|---|---|
| Direct eligible acting notice | URL + title; duplicate fragments canonicalized | Static public text with project, supporting role, SAG-AFTRA, Black woman 25–40, English, self-tape, future deadline | `Acting Role` | Deep-parser value is copied into the run candidate report; fixture routing uses `82` | Configured actor yields `demographic_match_status="Match"`; self-tape travel is zero; union is score-only | Opportunity is `visible`; report is `Accepted` | Opportunity, role/parse children, source suggestion, and one successful `DiscoveryRun` commit atomically | Bounded provider evidence survives in the response; canonical `original_post_url` and regenerated `source_url` survive on the Opportunity, but provider title/snippet/date do not persist there | Separate `Suggested`, unapproved domain candidate linked to opportunity | Bounded provider evidence is not yet durable |
...
| Hidden review | Parsed candidate whose created Opportunity is `hidden` | Supported | Usually acting with date/location uncertainty | Characterized report carries `68` | Existing services set hidden reason/rule | Result counts `hidden=1` and reports `Needs Review` with a stable reason | Hidden Opportunity commits with the successful run | URL plus current Opportunity metadata | No suggestion because only visible newly created results suggest sources | Bounded provider evidence remains response-scoped |
...
3. **Provider evidence is report-scoped rather than durable.** The result adapter keeps bounded title/snippet/date evidence in the candidate report, but `_ensure_details` does not persist that evidence on the Opportunity and the successful public-web run does not store it.
4. **Duplicate evidence and sightings are not merged.** There is no times-seen update for public-web Opportunity duplicates, and the dormant source-exclusion system is not involved.
5. **Candidate decisions and aggregates are semantically inconsistent.** A duplicate existing visible Opportunity still increments visible while its report is rejected.
6. **Union compatibility is not an eligibility rule.** It influences quality scoring; query targeting and future policy must define the intended behavior before changing it.
