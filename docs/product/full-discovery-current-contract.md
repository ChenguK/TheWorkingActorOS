...
Task 10 retains those bounds while making each layer ask for an individual notice using combinations of `casting notice`, `audition notice`, `seeking actor`/`seeking performer`, `role`/`character roles`, `submit`, `submission deadline`, `shoot date`, and configured self-tape terms. The planner still uses six or fewer deterministic layers rather than expanding profile dimensions into combinations, and it uses no undocumented provider-specific exclusion syntax.

## Public page-kind contract

`backend/app/automation/discovery/public_web_search.py::classify_public_web_page` runs on bounded fetched visible text before role normalization or page-wide demographic, role-type, travel, or trust evaluation. Its deterministic categories are `direct_opportunity`, `multi_listing_index`, `casting_resource`, `protected_page`, `irrelevant_page`, and `uncertain`. Specific employment evidence takes hard precedence, followed by resource/directory structure; a direct page requires a coherent project, role/seeking, and actionable submission, audition, shoot, or compensation signal. One production with multiple roles remains a direct opportunity, while repeated unrelated cards and browse/filter framing identify an index.

Only `direct_opportunity` continues through the existing normalization and Task 5 decision policy. `multi_listing_index` and `uncertain` receive report-scoped `review_hidden` outcomes without creating an Opportunity from the whole page. A `casting_resource` is discarded as non-actionable and remains distinct from source approval. `irrelevant_page` is discarded. Protected/login/CAPTCHA fetch results retain the Task 4 fetch rejection and are only labeled `protected_page`; fetch policy, reason code, and size limits are unchanged.
...
Trust date verification delegates supported-date evaluation to `BreakdownDeadlineService.validate_opportunity`. It therefore recognizes parseable dates from scalar Opportunity fields, `extracted_facts.dates`, `role_details`, and `production_details`, matching deadline evaluation. A valid structured future deadline is date evidence even when scalar columns remain empty; missing or malformed values retain the existing missing-date warning, and expired deadlines remain subject to the existing rejection path. This corrective contract does not complete the broader Task 8 vertical-slice verification.
...
Candidate identity remains the normalized HTTP(S) URL with fragments removed. Duplicate canonical URLs retain the first valid result's evidence deterministically; later titles, snippets, and dates are neither merged nor substituted. The candidate report carries evidence separately under `provider_evidence`, and its canonical URL is also the candidate source URL. The snippet is labeled as search-result context rather than verified fetched-page content.

## Durable run-scoped provider evidence

A successful configured public-web run stores evidence on `DiscoveryRun.run_payload.public_web_evidence` as `{"version": 1, "items": [...]}`. Each item uses its canonical URL as the stable candidate association and contains only `provider`, `canonical_url`, and present optional `title`, `snippet`, and `published_date` fields. The Task 3 field limits remain 80, 1,000, 240, 600, and 10 characters respectively. Ordering is provider-result order, canonical URL duplicates retain the first valid item without merging, and no more than the existing Parallel result budget of 10 items is retained. Ten fully populated maximum-length items have a compact nested JSON ceiling of 20,137 characters.

The serializer independently rechecks URL validity, fragments, field lengths, plain-text sanitation, dates, credential-looking content, JSON-like nested content, and actor name/private-note sentinels. It never serializes dataclass internals or arbitrary provider fields. Raw responses, fetched content, HTML, request/session IDs, undocumented result IDs, rankings, headers, cookies, credentials, DNS/network details, actor data, and provider/query request payloads are excluded. Opportunity and SourceResearchItem metadata remain unchanged.

Successful no-result runs store the explicit version-1 container with an empty `items` list. Failed runs use the evidence-free base payload, so provider, processing, serialization, flush, or commit failure cannot leave partial evidence. Historical payload dictionaries without `public_web_evidence` remain valid and require no migration. No API exposes `DiscoveryRun` records or `run_payload` directly, and the existing discovery-run response remains unchanged; no frontend type or behavior changes are required.
...
5. `_fetch_visible_text` fetches static HTML/plain content. `classify_public_web_page` first separates a direct notice from a listing index, resource, irrelevant page, or uncertain page. Direct notices alone reach `_normalize_candidate`, which runs `automation/discovery/classification.py::classify_breakdown_text`, rejects `Non-Acting Job`, `Crew Job`, and `Unknown`, and delegates field parsing to `services/breakdown_details_service.py::BreakdownDetailsService.from_fields`.
...
10. Registry `run_source` and the configured public-web orchestration boundary each own their transaction. One configured public-web execution creates one `DiscoveryRun`; accepted-visible and review-hidden Opportunities, travel exceptions, intended source suggestions, and bounded run-scoped provider evidence commit together at the single success boundary. Newly created rejected-discarded Opportunities are deleted before commit, while their provider evidence may remain in the successful run record. A systemic or evidence-serialization failure rolls back all partial writes, then best-effort persists exactly one failed run with no evidence from a clean transaction without masking the original exception. Disabled/skipped public-web behavior does not create a run.
...
|---|---|---|---|---|---|---|---|---|---|---|
| Direct eligible acting notice | URL + title; duplicate fragments canonicalized | Static public text with project, supporting role, SAG-AFTRA, Black woman 25–40, English, self-tape, future deadline | `Acting Role` | Deep-parser value is copied into the run candidate report; fixture routing uses `82` | Configured actor yields `demographic_match_status="Match"`; self-tape travel is zero; union is score-only | Opportunity is `visible`; report is `Accepted` | Opportunity, role/parse children, source suggestion, evidence, and one successful `DiscoveryRun` commit atomically | Bounded evidence is durable only on the run; canonical `original_post_url` and regenerated `source_url` survive on the Opportunity, but provider title/snippet/date do not persist there | Separate `Suggested`, unapproved domain candidate linked to opportunity | Opportunity metadata remains evidence-free |
| Ambiguous article/resource | URL + title | Static article mentions actors, auditions, roles, self tapes, but no actionable role/deadline | Page kind is `uncertain` or `casting_resource` before role normalization | Role-level confidence and eligibility are not applied to the whole page | Not reached | Uncertain pages receive report-scoped `review_hidden`; resources are discarded as non-actionable | No Opportunity is created from the whole page | Bounded provider title/snippet/date remain report/run evidence | None | Deeper extraction of individual linked notices is not implemented |
| Staff/crew listing | URL + optional title | Static staff-job text | `Non-Acting Job` or `Crew Job`; `_normalize_candidate` returns `None` | None | Not reached | Fetch-stage report remains `Rejected` with `Not an acting role` | No Opportunity or source suggestion | Bounded provider evidence remains in the response and successful run payload | None | Source-exclusion gate is not connected; rejection is recalculated on later runs |
| Expired notice | Normalized acting result with explicit past submission deadline | Supported text | Actor-facing classification may succeed | Not calculated/persisted because deadline rejects first | Not reached | `_process_public_web_search_result` reports `Rejected` / `Expired`; `deadline_expired=1` | No Opportunity or source suggestion | Bounded provider evidence can remain in the response and successful run payload | None | Past Opportunities are not retained for audit/history |
...
| One-off useful post | Exact public post URL | Supported static text | Acting | Parser-derived | Eligible | Can become `visible` without source approval | Durable Opportunity plus separate durable source suggestion in the successful run transaction | Exact post URL retained | Domain suggestion is `Suggested`, unapproved, inactive | Source approval remains deliberately separate from opportunity acceptance |
| Hidden review | Parsed candidate whose created Opportunity is `hidden` | Supported | Usually acting with date/location uncertainty | Characterized report carries `68` | Existing services set hidden reason/rule | Result counts `hidden=1` and reports `Needs Review` with a stable reason | Hidden Opportunity and run-scoped evidence commit with the successful run | URL plus current Opportunity metadata; bounded provider fields stay on the run | No suggestion because only visible newly created results suggest sources | Opportunity metadata remains evidence-free |
| Travel exception | In-person candidate outside travel policy but otherwise retained | Supported | Acting | Characterized report carries `82` | Travel evaluator returns `travel_exception` | `travel_exceptions=1`, `rejected=0`; report uses `review_hidden` | Travel-exception Opportunity commits with the successful run | URL plus travel metadata when real evaluator runs | No suggestion | The model has no separate travel-exception run counter |
| Discarded eligibility result | Parsed acting candidate with hard demographic/role/availability/travel failure | Supported | Acting | Characterized report carries `82` | Hard eligibility sets `discarded` and an exact `hidden_by_rule` | `discarded=1`, `rejected=1`; report carries the stable decision reason | A newly created rejected Opportunity is deleted before the successful run commits | Response-scoped report only | No suggestion | Rejection is counted without retaining an Opportunity row |
...
`backend/tests/contract_smoke/test_public_web_discovery_contract.py` proves the public-web boundary:

- successful and no-result executions persist exactly one successful run;
- accepted and reviewable Opportunities plus intended source suggestions are durable across sessions;
- rejected candidates remain successful-run outcomes but do not retain newly created Opportunity rows;
- provider, database, and commit failures roll back partial writes and persist one failed run when possible;
- a failed-run persistence error does not mask the original error and the session remains usable;
...
All Task 8 fixtures use fictional local data, sentinel configuration, mocked provider results, and mocked public fetches. They require no `.env` credential and make no provider or public-network call; deployed Render and Netlify configuration remains unchanged. Real Parallel recall and result quality, real-world public-page fetch compatibility, and social-platform coverage constrained by public accessibility and platform rules remain only partially verified, and this contract does not claim that the system finds all auditions.

## Sanitized first-canary regression

The explicitly authorized first canary used 5 bounded actor-specific queries, one Parallel call, and `max_results=10`. Its 10 results produced 0 visible, 1 review, and 9 discarded outcomes. The dominant pattern was directory/category/resource/landing pages plus protected or bot-challenged pages, rather than individual actionable notices; aggregate observations also included two oversized responses. No raw provider payload or fetched body is retained here. Task 10's query intent and page-kind boundary are intended to improve direct-notice precision before any separately authorized second live canary; they do not increase the call, result, or fetch-size budgets.
...
1. **Review-only page kinds do not create a durable review Opportunity.** Directory and uncertain-page outcomes are explicit in the run report, but deeper supported extraction of their individual notices is deferred.
2. **Page-kind classification is deterministic and intentionally conservative.** Real pages may omit labels or use novel directory structures, so a second authorized canary is still required to measure precision and recall.
3. **Duplicate evidence and sightings are intentionally not merged.** Run-scoped evidence keeps the first canonical-URL result, there is no times-seen update for public-web Opportunity duplicates, and the dormant source-exclusion system is not involved.
4. **Candidate decisions and aggregates are semantically inconsistent.** A duplicate existing visible Opportunity still increments visible while its report is rejected.
5. **Union compatibility is not an eligibility rule.** It influences quality scoring; query targeting and future policy must define the intended behavior before changing it.

These gaps are evidence for later tasks.
